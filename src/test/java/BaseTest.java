...
public class BaseTest {
    ...
    public static void navigateToAllSongs() {
        WebElement navigateToAllSongsPage = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//a[@href='#!/songs']")));
        navigateToAllSongsPage.click();
    }
    public static void songDoubleClick() {
        Actions act = new Actions(driver);
        WebElement songDoubleClick = wait.until(ExpectedConditions.elementToBeClickable(By.cssSelector("[class='song-item']")));
        act.doubleClick(songDoubleClick).perform();
    }
    public static void songIsPlaying() {
        WebElement songPlaying = wait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("[alt='Sound bars']")));
    ...
    public static void enterPlaylistName() {
        WebElement playlistInput = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//input[@name='name']")));
        playlistInput.sendKeys("My playlist");
        playlistInput.sendKeys(Keys.ENTER);
    }
    public static void verifyPlaylistCreated() {
        WebElement student = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//a[text()='My playlist']")));
        Assert.assertTrue(student.isDisplayed());
    }
    public static void selectSong() {
        WebElement song = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//section[@id='playlistWrapper']//td[@class='title']")));
        song.click();
    }
    public static void deleteSong() {
        WebElement songDelete = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//table[@class='items']']")));
        songDelete.click();
        songDelete.sendKeys(Keys.BACK_SPACE);
        //Actions actn = new Actions(driver);
       // actn.keyDown(Keys.DELETE);
    }
    ...
    public static void selectPlaylist() {
        WebElement mPlaylist = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//li[@class='playlist']")));
        mPlaylist.click();
    }
    public static void open_m_Playlist() {
        WebElement playlist = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//a[text()='m']")));
        playlist.click();
    }
    public static void verifySongIsAdded() {
        WebElement songPlaylist = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//*[@id='playlistWrapper']/div/div/div[1]/table/tr[2]/td[2]")));
        songPlaylist.click();
        Assert.assertTrue(songPlaylist.isDisplayed());
    }
    public static void openPlaylistPage() {
        WebElement myPlaylist = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//a[text()='My playlist']")));
        myPlaylist.click();
    }
    public static void clickDeleteBtn() {
        WebElement playlistDelete = wait.until(ExpectedConditions.elementToBeClickable(By.cssSelector("[class='del btn-delete-playlist']")));
        playlistDelete.click();
    }
    public static void verifySuccessDeleted() {
        WebElement playlistDelete = wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector("[class='success show']")));
